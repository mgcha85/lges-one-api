...
const CHANNEL_OPTION_KO_MAP = {
  'OpenAI 兼容': 'OpenAI 호환',
  'OpenAI 兼容渠道，支持设置 Base URL': 'Base URL 설정을 지원하는 OpenAI 호환 채널',
  'Gemini OpenAI 兼容格式': 'Gemini OpenAI 호환 형식',
  '字节火山引擎': 'ByteDance Volcano Engine',
  '原字节跳动豆包': '기존 ByteDance Doubao',
  '百度文心千帆': 'Baidu Wenxin Qianfan',
  '百度文心千帆 V2': 'Baidu Wenxin Qianfan V2',
  '阿里通义千问': 'Alibaba Tongyi Qianwen',
  '阿里云百炼': 'Alibaba Cloud Bailian',
  '讯飞星火认知': 'iFlytek Spark',
  '讯飞星火认知 V2': 'iFlytek Spark V2',
  '智谱 ChatGLM': 'Zhipu ChatGLM',
  '360 智脑': '360 Brain',
  ...
  '阶跃星辰': 'Stepfun',
  '自定义渠道': '사용자 정의 채널',
  '知识库：FastGPT': '지식 베이스: FastGPT',
  '知识库：AI Proxy': '지식 베이스: AI Proxy',
  '代理：API2D': '프록시: API2D',
  '代理：OpenAI-SB': '프록시: OpenAI-SB',
  '代理：OhMyGPT': '프록시: OhMyGPT',
  '代理：AI Proxy': '프록시: AI Proxy',
  '代理：CloseAI': '프록시: CloseAI',
  '代理：OpenAI Max': '프록시: OpenAI Max',
  '代理：AI.LS': '프록시: AI.LS',
  '代理：API2GPT': '프록시: API2GPT',
  '代理：AIGC2D': '프록시: AIGC2D',
};

function localizeChannelOption(option, language) {
  if (language !== 'ko') {
    return option;
  }
  ...
  return {
    ...option,
    text: CHANNEL_OPTION_KO_MAP[option.text] || option.text,
    description:
      CHANNEL_OPTION_KO_MAP[option.description] || option.description,
  };
}
...
const EditChannel = () => {
  ...
  const currentLanguage = (i18n.resolvedLanguage || i18n.language || 'zh')
    .toLowerCase()
    .split('-')[0];
  const localizedChannelOptions = CHANNEL_OPTIONS.map((option) =>
    localizeChannelOption(option, currentLanguage)
  );
