...
function renderTimestamp(timestamp, request_id, t) {
  return (
    <code
      onClick={async () => {
        if (await copy(request_id)) {
          showSuccess(
            t('log.messages.copy_request_id_success', {
              request_id,
              defaultValue: 'Request ID copied: {{request_id}}',
            })
          );
        } else {
          showWarning(
            t('log.messages.copy_request_id_failed', {
              request_id,
              defaultValue: 'Failed to copy request ID: {{request_id}}',
            })
          );
        }
      }}
    ...
    default:
      return (
        <Label basic color='black'>
          {t('log.type.unknown', { defaultValue: 'Unknown' })}
        </Label>
      );
  }
}
            ...
            .map((log, idx) => {
              ...
              return (
                <Table.Row key={log.id}>
                  <Table.Cell>
                    {renderTimestamp(log.created_at, log.request_id, t)}
                  </Table.Cell>
                  {isAdminUser && (
                    <Table.Cell>
                      {log.channel ? (
